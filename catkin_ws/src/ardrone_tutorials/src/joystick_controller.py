...
def ReceiveJoystickMessage(data):
	...
	if (DroneState == False):
		...
		if data.buttons[ButtonEmergency]==1:
			...
			rospy.loginfo("Axis: %s",data.axes)
...
def face_wind(wind_dir):
	global phase
	if (abs(wind_dir) <=30):
		phase = 3
		controller.SetCommand(0/ScaleRoll,0/ScalePitch,0/ScaleYaw,0/ScaleZ)
		return
	else :
		controller.SetCommand(0/ScaleRoll,0/ScalePitch,0.1/ScaleYaw,0/ScaleZ)
		return
...
if __name__=='__main__':
	...
	#~ display = DroneVideoDisplay(Group)
	controller = BasicDroneController(Group)
	...
	while not rospy.is_shutdown():
		rospy.loginfo("Battey: %s",controller.bat)
		rospy.loginfo("Wind_Speed: %s, Wind_Angle: %s, Wind_Comp_Angle: %s",controller.ws,controller.wa,controller.wca)
		rospy.loginfo("Velocity_X: %s, Velocity_Y: %s, Velocity_Z: %s, Altitude: %s",controller.vel[0],controller.vel[1],controller.vel[2],controller.altitude)
		rospy.loginfo("Rot_X: %s, Rot_Y: %s, Rot_Z: %s",controller.rot[0],controller.rot[1],controller.rot[2])
		if (DroneState) :
			Auto(controller.wa)
		sleep(1)
